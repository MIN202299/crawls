...
解题思路：
    1. chrome search 没有发现有价值的东西
    2. 通过请求的函数执行栈发现了可疑代码，通过在线JS反混淆工具成功解决 解密工具https://www.sojson.com/jsjiemi.html
...
"""
js 代码解密前
window['\x75\x72\x6c'] = '\x2f\x61\x70\x69\x2f' + '\x6d\x61\x74\x63\x68' + '\x2f\x31', request = function () {
...
window['url'] = '/api/' + 'match' + '/1', request = function() {
    // oo0O0这是一个可疑函数 分析发现这个函数的js脚本在首页数据中 接下来就好办了
...
def getHomePage():
    ...
    res = requests.get(url, headers=getHeaders(), timeout=5)
    if res.status_code == 200:
        html = etree.HTML(res.text)
        print(res.text)
        with open('inspirations/01.js', 'w', encoding='utf-8') as f:
            # 关键代码一 关键函数 oo0O0
            key_code1 = html.xpath('/html/body/script[8]/text()')[0] + '\n'
            # 关键代码二 关键常量
            key_code2 = html.xpath('/html/body/script[5]/text()')[0] + '\n'
            # 关键代码三 window.a
            key_code3 = html.xpath('/html/head/script[9]/text()')[0] + '\n'
            # 关键代码四 w() dd()
            key_code4 = html.xpath('/html/head/script[10]/text()')[0] + '\n'
            # 关键代码五 eval()
            key_code5 = html.xpath('/html/body/script[9]/text()')[0] + '\n'

            # 其实这些可以直接通过chrome控制台打印
            f.writelines([key_code1, key_code2, key_code3, key_code4, key_code5])


def getJSONData(page, m):
    url = 'https://match.yuanrenxue.com/api/match/1'
    params = {
        'page': page,
        'm': m
    }
    res = requests.get(url, params=params, headers=getHeaders(page), timeout=5)
    if res.status_code == 200:
        print(res.text)
